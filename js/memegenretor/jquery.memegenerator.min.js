(function(c) {
    var b = {
        topTextPlaceholder: "TOP TEXT HERE",
    ...
    var a = function(e, d) {
        ...
        this.ui = {
            createControls: function() {
                var g = c('<div class="mg-controls content-box text-black sm-mt-30"></div>');
            ...
            createTextBox: function(m, g) {
                ...
                //var sk = c('<div>Hello I am calling Here</div>');
                ...
                if(h!='layer1'){
                i.append(c('</div><div class="meme_text_setting col-md-5">'+
                '<select id="meme_lang" class="form-control" style="float: left; margin-right: 15px;">'+
                '<option value="english">English</option>'+
                '<option value="hindi">Hindi</option>'+
                '<option value="tamil">Tamil</option>'+
                '<option value="telugu">Telugu</option><option value="kannada">Kannada</option><option value="malayalam">Malayalam</option></select>'+
                '</div>'));
                }
                ...
                i.append(c('<select style="float:left; margin-left:15px;" class="mg-textbox-font form-control col-md-4">'+
                '<option value="BalooTamma">Baloo Tamma</option>'+
                '<option value="BalooPaaji">Baloo Paaji</option>'+
                ...
                '<option value="Lily Script One">Lilt one</option>'+
                ...
                if(h!='layer1'){
                    i.append(c('<div class="meme_text_setting col-md-8">'+
                    '<a role="button" class="btn btn-default mg-text-bold">B</a>'+
                    '<a role="button" class="btn btn-default mg-text-uppercase">ABC</a>'+
                    '<a role="button" class="btn btn-default mg-text-italic"><i>abc</i></a>'+
                    '<a role="button" class="btn btn-default mg-text-lowercase">abc</a><a style="display:none;" class="resetmeme">reset</a>'+
                    '</div><div class="form-group memetags col-md-12"><input type="text" name="meme_name" placeholder="Meme Name" id="meme_name" required="" class="form-control"></div>'+
                    '<div class="form-group col-md-12"><input type="text" name="meme_tag" placeholder="Add tags (multi-word allowed, enter to confirm)"  class="form-control" id="meme_tag" required=""></div>'+
                    '<div class="form-group text-uppercase col-md-12"><button class="btn btn-success btn-lg mr-10" id="create_meme">Create Meme</button><button class="btn btn-danger btn-lg mr-10" id="loading-image" style="display:none;"><i class="fa fa-spinner fa-spin"></i> Loading</button><a class="btn btn-primary text-capitalize reset_meme">Reset <i class="fa fa-refresh" aria-hidden="true"></i></a></div>'+
                    '<div class="form-group col-md-12"><a data-toggle="modal" data-target="#myModal1" class="text-blue" href="#">Somthing odd? Report this image</a><br><a class="text-blue" href="#">Upload Images</a></div>'));
                }
                f.canvasLayers.push(h);
                i.find(".mg-textbox-text").on("inputchange keydown", function(e) {
                ...
                i.find(".mg-textbox-font").on("inputchange", function() {
                ...
                i.find(".mg-text-lowercase").on("click", function() {  
                        ...
                        //console.log(f.ui._strtolower(c('.layer1 .mg-textbox-text').val()));
                        f.events.onLayerChange(h)
                });
                ...
                i.find(".mg-text-bold").on("click", function() {  
                    //alert('yes');
                        if(f.settings.defaultTextStyle.weight=='normal'){
                            ...
                            //console.log(f.settings.defaultTextStyle);
                        }
                        else{
               ...
               i.find(".resetmeme").on("click", function() { 
                f.settings.defaultTextStyle.style='normal';
                f.settings.defaultTextStyle.color="#ffffff";
                f.settings.defaultTextStyle.size=80;
                f.settings.defaultTextStyle.font='BalooTamma';
                f.settings.defaultTextStyle.weight='normal';
                f.settings.defaultTextStyle.forceUppercase=true;
                $('.mg-textbox-text').val('');
                $('.sp-preview-inner').css("background-color", "white");
                $('.meme_text_setting a').removeClass('active');
                $('#meme_lang option').prop('selected', function() {
                     return this.defaultSelected;
                });
                $('.mg-textbox-font option').prop('selected', function() {
                     return this.defaultSelected;
                });
                f.events.onLayerChange(h);
               });
                ...
                i.find(".mg-textbox-text-color").on("change", function() {
                       f.settings.defaultTextStyle.color=this.value;
                       f.events.onLayerChange(h);
            ...
            createPositionHelper: function(i) {
                var g = i.data("layer");
                var h = c('<div class="draggable" data-target-layer="' + g + '"></div>').appendTo(f.helpersContainer);
                h.css({
            ...
            bootstrapify: function() {
                ...
                g.find(".mg-textbox").each(function() {
                    if (!c(this).hasClass("row")) {
                        c(this).addClass("row");
                        c(this).find(".mg-textbox-text").addClass("form-control").wrap(c("<div></div>").addClass("col-md-12"));
                                               
                        c(this).find(".mg-textbox-size").addClass("form-control").wrap(c("<div></div>").addClass("col-md-3 meme_fonsize"));
                        ...
                        c(this).find(".colorpickerContainer").addClass("col-md-3");
        ...
        this.canvas = {
                ...
                f.canvasLayers.forEach(function(i) {
                    ...
                    if (j.hasClass("mg-textbox")) {
                        var l = {
                            ...
                            fontSize: f.settings.defaultTextStyle.size * g,//j.find(".mg-textbox-size").val() * g,
                            font: f.settings.defaultTextStyle.font,//j.find(".mg-textbox-font").val(),
                            color: f.settings.defaultTextStyle.color,//j.find(".mg-textbox-text-color").val(),
                            style: f.settings.defaultTextStyle.style,
                            weight: f.settings.defaultTextStyle.weight,
                            borderWidth: f.settings.defaultTextStyle.borderWidth * g
                            
                        };
                        
                        var h = f.canvas.drawText(l.text, l.x, l.y, l.maxWidth, l.fontSize, l.font, l.color, l.style, l.weight,l.borderWidth, g);
            ...
            drawText: function(t, q, o, s, v, j, m, k, h, b, l) {
